...
export function verifyToken(token: string): AuthPayload | null {
  try {
    ...
    // iat is carried through: session revocation compares it against the
    // account's sessionsRevokedAt.
    return { sub: String(decoded.sub), role, iat: decoded.iat, exp: decoded.exp };
  } catch {
    return null;
  }
}

      ...
      if (role === 'pet_parent') {
        ...
        try {
          p = await prisma.petParent.findUnique({
            ...
            select: { id: true, name: true, phone: true, email: true, city: true, country: true, emailVerified: true, emailVerifiedAt: true, digestFrequency: true, sessionsRevokedAt: true },
          });
        } catch (err) {

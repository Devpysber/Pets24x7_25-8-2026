...
export async function createOrder(opts: {
  ...
  mobileNumber?: string;
  /**
   * Where PhonePe returns the payer. Defaults to the membership return page;
   * campaign and featured checkouts pass their own, or the vendor lands on a
   * page that polls the wrong endpoint.
   */
  returnUrl?: string;
}): Promise<{ redirectUrl: string; raw: PayResponse }> {
  const base = opts.returnUrl || env.PHONEPE_REDIRECT_URL;
  const payload: PayPayload = {
    merchantId: env.PHONEPE_MERCHANT_ID,
    merchantTransactionId: opts.merchantTxnId,
    merchantUserId: opts.parentId,
    amount: opts.amountMinor,
    redirectUrl: base + (base.includes('?') ? '&' : '?') + 'txn=' + encodeURIComponent(opts.merchantTxnId),
